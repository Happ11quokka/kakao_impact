...
export default function Workshop() {
  const { slot1, slot2, setSlot, combine, clearSlots, lastResult, clearResult } = useCraftingStore();
  const { gems, fetchInventory, addGem, consumeGem } = useInventoryStore();
  ...
  const availableGems = gems.filter(g => !g.consumedAt && g.id !== slot1?.id && g.id !== slot2?.id);
  ...
  const handleCraft = () => {
    ...
    setTimeout(() => {
      ...
      if (result.success && result.resultGem) {
        ...
        if (slot1) consumeGem(slot1.id);
        if (slot2) consumeGem(slot2.id);
        addGem(result.resultGem);
        setShowResult(true);
      }
  ...
  return (
    <div style={{
      ...
      {/* 플래시 이펙트 */}
      {showFlash && (
        <div className="animate-craft-flash" style={{
          position: 'absolute', inset: 0, zIndex: 50,
          background: 'radial-gradient(circle, rgba(255,215,0,0.8) 0%, rgba(232,97,77,0.4) 40%, transparent 70%)',
          pointerEvents: 'none',
        }} />
      )}
      ...
      <div style={{ padding: '16px 16px 0', textAlign: 'center', zIndex: 2 }}>
        <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 20, fontWeight: 700, color: '#E8D8C8' }}>⚒️ 세공소</h1>
      </div>

      {/* 메인 세공 패널 */}
      <div style={{
        flex: 1, display: 'flex', flexDirection: 'column', zIndex: 2,
        margin: '12px 14px', borderRadius: 'var(--radius-lg)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginBottom: 16 }}>
          ...
          <div style={{
            ...
            color: '#E8A838', fontSize: 18, fontWeight: 700, flexShrink: 0,
          }}>+</div>
        ...
        {canCraft && (
          <div className="animate-scale-pop" style={{
            ...
            {craftable ? (
              <>
                <div style={{ fontSize: 11, color: 'rgba(150,255,150,0.7)', marginBottom: 8 }}>결과 미리보기</div>
                <div style={{
                  ...
                  {isSameType && slot1 ? (
                    <GemStone gem={{ ...slot1, tier: Math.min(4, slot1.tier + 1) as GemTier, id: 'preview' }} size={32} />
                  ) : recipe ? (
              ...
              <>
                ...
                <div style={{ color: '#FF8888', fontSize: 13, fontWeight: 600 }}>이 조합으로는 세공할 수 없어요</div>
              </>
            )}
          </div>
        )}
        ...
        <button
          ...
          style={{
            ...
            padding: '14px 48px', borderRadius: 'var(--radius-full)',
            border: canCraft && craftable ? '2px solid rgba(232,97,77,0.5)' : '2px solid rgba(100,100,120,0.3)',
            background: canCraft && craftable
              ? 'linear-gradient(135deg, #E8614D 0%, #D45440 100%)'
              : 'rgba(60,60,80,0.5)',
            color: canCraft && craftable ? 'white' : 'rgba(150,150,170,0.5)',
            fontWeight: 700, fontSize: 16, cursor: canCraft && craftable ? 'pointer' : 'not-allowed',
            boxShadow: canCraft && craftable ? '0 4px 20px rgba(232,97,77,0.4), inset 0 1px 0 rgba(255,255,255,0.2)' : 'none',
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginTop: 16 }}>
          {(slot1 || slot2) && (
            <button onClick={() => { clearSlots(); clearResult(); }}
              style={{ background: 'none', border: 'none', color: 'rgba(200,200,220,0.5)', fontSize: 12, cursor: 'pointer' }}>
              🔄 슬롯 비우기
            </button>
          )}
        </div>
      ...
      {showPicker !== null && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 30 }}>
          <div onClick={() => setShowPicker(null)} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.5)' }} />
          <div className="animate-slide-up" style={{
            position: 'absolute', bottom: 0, left: 0, right: 0,
            background: '#252838', borderRadius: 'var(--radius-lg) var(--radius-lg) 0 0',
            padding: '20px 16px', maxHeight: '55%', overflow: 'auto',
            ...
            {availableGems.length === 0 ? (
              <p style={{ textAlign: 'center', color: 'rgba(200,200,220,0.5)', padding: 20 }}>사용 가능한 보석이 없어요</p>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
                {availableGems.map(gem => {
                  ...
                  return (
                    <div key={gem.id}
                      onClick={() => { setSlot(showPicker, gem); setShowPicker(null); clearResult(); }}
                      style={{
                        display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5,
                        padding: 10, borderRadius: 'var(--radius-md)',
                        background: 'rgba(60,65,90,0.6)', cursor: 'pointer',
                        border: `1px solid ${emotion?.hexColor}30`,
                        transition: 'all var(--duration-fast) var(--easing-out)',
                      }}
                    >
                      <GemStone gem={gem} size={30} />
                      <span style={{ fontSize: 10, color: 'rgba(200,200,220,0.7)' }}>{emotion?.nameKo}</span>
                      <span style={{ fontSize: 8, color: 'rgba(200,200,220,0.4)' }}>Lv.{gem.tier}</span>
                    </div>
                  );
                })}
              </div>
      ...
      {showResult && lastResult?.success && lastResult.resultGem && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 40, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={() => { setShowResult(false); clearResult(); }} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.7)' }} />
          ...
          <div className="animate-scale-pop" style={{
            ...
            <div style={{
              ...
              <GemStone gem={lastResult.resultGem} size={56} />
            </div>
            ...
            {lastResult.recipe && (
              <div style={{
                margin: '12px auto 0', padding: '6px 16px',
                borderRadius: 'var(--radius-full)',
                background: 'rgba(232,168,56,0.15)', border: '1px solid rgba(232,168,56,0.3)',
                color: '#E8A838', fontSize: 12, fontWeight: 600, display: 'inline-block',
              }}>
                🃏 {lastResult.recipe.nameKo}
              </div>
            )}
            <button onClick={() => { setShowResult(false); clearResult(); }}
              style={{
                display: 'block', margin: '24px auto 0', padding: '10px 36px',
                borderRadius: 'var(--radius-full)', border: '1px solid rgba(255,215,0,0.3)',
                background: 'rgba(60,65,90,0.6)', color: '#E8D8C8',
...
/** 아이템 카드형 슬롯 */
function SlotCard({ gem, label, onClick }: { gem: import('../types/gem').Gem | null; label: string; onClick: () => void }) {
  const emotion = gem ? getEmotion(gem.emotionCode) : null;

  return (
    <button onClick={onClick} style={{
      width: 120, minHeight: 140,
      borderRadius: 'var(--radius-md)',
      border: gem
        ? `2px solid ${emotion?.hexColor}60`
        : '2px dashed rgba(120,130,180,0.25)',
      background: gem
      ...
      {gem ? (
        <>
          ...
          <span style={{ color: '#E8D8C8', fontSize: 13, fontWeight: 700 }}>
            {emotion?.gemName}
          </span>
          <span style={{ color: 'rgba(200,200,220,0.6)', fontSize: 11 }}>
            {emotion?.nameKo}
          </span>
          <span style={{
