...
폴더는 기능 단위로 나뉘어 있습니다 — 어떤 기능이 어디에 있는지 기준으로 읽으면 됩니다.

```
kakao_impact/
├── 1_avoha/                1차 MVP 테스트 — 소확행 (투자자 데모용 모바일 웹앱)
│   ├── src/                지도 기반 하루 기록 + AI 자동 분석 목업 화면
│   └── tests/              유닛·프로퍼티 테스트
│
├── 2_Ulog/                 유로그(Ulog) — 메인 서비스
│   ├── frontend/           📱 PWA 웹앱 — 사용자가 보는 모든 화면
│   │                          홈(오늘의 마음·원석 세공) / 감정 분석(주간·월간 패턴, 리캡)
│   │                          캘린더 회고 / 캐릭터 도감 / 카카오 로그인
│   ├── backend/            ⚙️ API 서버 — 웹앱이 쓰는 모든 데이터
│   │                          원석 저장·세공·재분류 / 실시간 인벤토리(SSE)
│   │                          세션 인증 / DB 마이그레이션
│   ├── ai/
│   │   ├── chatbot/        🤖 카카오톡 챗봇 — 감정 채집의 입구 (라이브)
│   │   │                      2단계 AI 감정분류 / 자기인지 질문 / 결과 카드 회신
│   │   ├── agent/          사진 이벤트 그룹핑 에이전트 — 설계만, 미배포
│   │   ├── rembg/          사진 누끼(배경 제거) 서비스 — 스캐폴드, 미배포
│   │   └── ops/            프롬프트 회고·학습 데이터 추출 스크립트
│   ├── design/             🎨 브랜드·와이어프레임·픽셀 스프라이트(원석/로기) 에셋
│   └── ops/                🔧 운영 콘솔 + 데모 시딩·데이터 동기화 스크립트
│
└── docs/                   📚 시스템 아키텍처 · 데이터 분석 문서 · 포트폴리오 자산(영상/이미지)
```
