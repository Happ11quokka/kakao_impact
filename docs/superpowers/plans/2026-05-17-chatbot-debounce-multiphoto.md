...
| 파일 | 변경 내용 |
|------|---------|
| `2_avoha/ai/chatbot/main.py` | asyncio import 추가, 디바운스 상태변수, `_flush_simple_records`, 단순기록 핸들러 2곳, `_safe_pending_photo`, 사진 수신 로직, webhook 콜사이트 |
...
**Files:**
- Modify: `2_avoha/ai/chatbot/main.py:1-14` (import 블록)
- Modify: `2_avoha/ai/chatbot/main.py:54-62` (상태 변수 블록)
...
```bash
grep -n "SIMPLE_RECORD_DEBOUNCE_S\|import asyncio" 2_avoha/ai/chatbot/main.py
```
...
**Files:**
- Modify: `2_avoha/ai/chatbot/main.py:572` (save_gem 함수 끝 다음)

- [ ] **Step 1: `save_gem` 끝 위치 확인**

```bash
grep -n "^def save_gem\|^def _ensure_reflection" 2_avoha/ai/chatbot/main.py
```
...
```bash
grep -n "_flush_simple_records" 2_avoha/ai/chatbot/main.py
```
...
**Files:**
- Modify: `2_avoha/ai/chatbot/main.py:1655-1660`
...
```bash
grep -n "pending_simple_buffer\|pending_simple_timer" 2_avoha/ai/chatbot/main.py | head -20
```
...
**Files:**
- Modify: `2_avoha/ai/chatbot/main.py` — `"기록됐어요! "` 가 있는 단순기록 블록

- [ ] **Step 1: 현재 텍스트 핸들러 위치 확인**

```bash
grep -n '"기록됐어요' 2_avoha/ai/chatbot/main.py
```
...
```bash
grep -n "pending_simple_buffer\|pending_simple_timer\|pending_simple_callback" 2_avoha/ai/chatbot/main.py | wc -l
```
...
**Files:**
- Modify: `2_avoha/ai/chatbot/main.py:935-951` (`_safe_pending_photo`)
- Modify: `2_avoha/ai/chatbot/main.py` (사진 수신 저장 로직 — `print(f"[image detected]")` 이후 블록)
...
**Files:**
- Modify: `2_avoha/ai/chatbot/main.py` — `_safe_pending_photo` 호출 직후 (webhook 함수 하단)

- [ ] **Step 1: 콜사이트 위치 확인**

```bash
grep -n "_safe_pending_photo" 2_avoha/ai/chatbot/main.py
```
...
```bash
cd 2_avoha/ai/chatbot && python -c "from main import app; print('import OK')"
```
...
- 필요 라이브러리: `piexif` 추가 (`2_avoha/ai/chatbot/requirements.txt`)
