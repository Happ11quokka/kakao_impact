...
| 파일 | 변경 내용 |
|------|----|
| `2_Ulog/ai/chatbot/requirements.txt` | `piexif==1.1.3` 추가 |
| `2_Ulog/ai/chatbot/exif_grouping.py` (신규) | `parse_photo_exif`, `group_photos_by_event` 순수 함수 |
| `2_Ulog/ai/chatbot/main.py` | 상태 변수 + 단순기록/감정분류 핸들러 변경 + 다중 이벤트 진행 로직 |
...
**Files:**
- Modify: `2_Ulog/ai/chatbot/requirements.txt`
- Create: `2_Ulog/ai/chatbot/exif_grouping.py`

- [ ] **Step 1: piexif를 requirements.txt에 추가**

`2_Ulog/ai/chatbot/requirements.txt` 마지막 줄 다음에 추가:
```
piexif==1.1.3
```
...
`2_Ulog/ai/chatbot/exif_grouping.py` 신규 생성, 아래 내용으로:
...
python -c "
import ast
for fp in ['2_Ulog/ai/chatbot/main.py', '2_Ulog/ai/chatbot/exif_grouping.py']:
    with open(fp) as f:
        ast.parse(f.read())
print('syntax OK')
"
...
```bash
cd /Users/imdonghyeon/kakaoimpact
git add 2_Ulog/ai/chatbot/requirements.txt 2_Ulog/ai/chatbot/exif_grouping.py
...
**Files:**
- Modify: `2_Ulog/ai/chatbot/main.py`
...
```bash
grep -n 'pending_photo\[user_id\] = {"time":' /Users/imdonghyeon/kakaoimpact/2_Ulog/ai/chatbot/main.py
```
...
**Files:**
- Modify: `2_Ulog/ai/chatbot/main.py` (import 블록, 상태 변수 블록, helper 함수)

- [ ] **Step 1: exif_grouping 모듈 import 추가**

`2_Ulog/ai/chatbot/main.py` 상단의 `import psycopg2` 라인 다음에 추가:
...
**Files:**
- Modify: `2_Ulog/ai/chatbot/main.py` (단순기록 모드 사진 핸들러)
...
```bash
grep -n "사진이 바로 저장됐어요" /Users/imdonghyeon/kakaoimpact/2_Ulog/ai/chatbot/main.py
```
...
**Files:**
- Modify: `2_Ulog/ai/chatbot/main.py` (단순기록 모드 텍스트 핸들러)

- [ ] **Step 1: 핸들러 위치 확인**

```bash
grep -n '"기록됐어요' /Users/imdonghyeon/kakaoimpact/2_Ulog/ai/chatbot/main.py
```
...
**Files:**
- Modify: `2_Ulog/ai/chatbot/main.py` (메인 webhook 텍스트 처리부)
...
```bash
grep -n "has_photo, image_urls, photo_time = _safe_pending_photo" /Users/imdonghyeon/kakaoimpact/2_Ulog/ai/chatbot/main.py
```
...
**Files:**
- Modify: `2_Ulog/ai/chatbot/main.py` (webhook 함수 상단부, 특별 명령어 처리 이후)
...
```bash
grep -n "    if not utterance:" /Users/imdonghyeon/kakaoimpact/2_Ulog/ai/chatbot/main.py
```
...
**Files:**
- Modify: `2_Ulog/ai/chatbot/main.py` ("맞아요" 핸들러 직후 응답 반환 전)
...
```bash
grep -n '    # 맞아요 (저장)' /Users/imdonghyeon/kakaoimpact/2_Ulog/ai/chatbot/main.py
```
...
**Files:**
- Modify: `2_Ulog/ai/chatbot/main.py` (webhook 진입부 초반에 가드)
...
```bash
grep -n "user_id, utterance, callback_url = _extract_kakao_request" /Users/imdonghyeon/kakaoimpact/2_Ulog/ai/chatbot/main.py
```
...
import ast
with open('2_Ulog/ai/chatbot/exif_grouping.py') as f:
    ast.parse(f.read())
...
```bash
cd /Users/imdonghyeon/kakaoimpact/2_Ulog/ai/chatbot && python -c "from main import app; print('import OK')" 2>&1 | tail -3
```
