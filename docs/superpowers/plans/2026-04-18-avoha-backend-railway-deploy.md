...
이번 배포 작업은 **config/infra 변경**이 대부분이다. 현재 `2_avoha/backend` 에는 테스트 러너(`vitest`/`jest`)가 설치돼 있지 않고, MVP 범위에서 테스트 인프라를 새로 들이는 건 over-engineering 이다. 따라서:
...
### 수정 파일
- `2_avoha/backend/package.json` — `start` 스크립트에서 `--env-file=.env` 제거
- `2_avoha/backend/src/env.ts` — `REDIS_URL` optional 로 변경
...
**Files:**
- Modify: `2_avoha/backend/package.json:12`
- Modify: `2_avoha/backend/src/env.ts:7`
...
git add 2_avoha/backend/package.json 2_avoha/backend/src/env.ts
...
같은 Source 섹션에서:
- `Root Directory`: `2_avoha/backend`
...
**Files:**
- Modify: `2_avoha/backend/src/routes/health.ts`
...
```bash
cd /Users/imdonghyeon/kakaoimpact
git add 2_avoha/backend/src/routes/health.ts
